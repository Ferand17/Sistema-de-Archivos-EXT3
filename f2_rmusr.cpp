...
f2_rmusr::f2_rmusr(Lista_Particiones *lista, int *idUsuario, QString *usuario, QString *id_mount, QString *grupo, int *idGrupo, nodo_particiones *part_global){
    this->lista=lista;
...
void f2_rmusr::ast_rmusr(Nodo_arbol *raiz){
    string log_error;
    bool berror=false;
    bool busr=false;
    QString vusr="";
...
void f2_rmusr::ejecutar_rmusr(QString pusr){
    if(*idUsuario == 1){
        ...
        if (disco != NULL){
            ...
            if(part_global->part_type == 'P'){
                ...
                int resp = archivo.eliminarUsuario(pusr);
                if(resp ==  0 ){cout<<"\033[96mEl usuario se eliminó correctamente.\033[0m"<<endl<<endl;}
                else if(resp == 1){cout<<"\033[31mERROR: El nombre usuario no existe.\033[0m"<<endl<<endl;}
                else{cout<<"\033[31mERROR: No se pudo eliminar el usuario.\033[0m"<<endl<<endl;}
            }
...
string f2_rmusr::ReplaceAll(string str, const string& from, const string& to){
    size_t start_pos = 0;
